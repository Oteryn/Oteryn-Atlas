...
- Heavy Molehill qualification must enter through `e2e/run.ps1`, which owns a bounded machine-wide slot pool for isolated concurrent full 73-scenario gates. The measured safe default is 2 slots; 3 remains explicit opt-in capacity only. Each active slot must keep a unique Compose project/artifact namespace and independent local publication forwarder; legacy single-lock runners remain fenced during migration. Do not bypass the pool or exceed the repository-selected slot capacity.
