...
- Heavy Molehill qualification must be serialized through `e2e/run.ps1`; never launch concurrent 67-scenario local gates or competing full browser-depth runs against the same publication origin. The wrapper holds a machine-wide exclusive lock so agents wait instead of overloading the publication path.
...
- Synology must not run the 67-scenario full PR matrix, broad stress matrices, soak, performance depth or visual-regression depth as a substitute for Molehill-PC capacity.
