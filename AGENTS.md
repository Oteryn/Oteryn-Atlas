...
- Heavy Molehill qualification must be serialized through e2e/run.ps1; never launch concurrent 60-scenario local gates or competing full browser-depth runs against the same publication origin. The wrapper holds a machine-wide exclusive lock so agents wait instead of overloading the publication path.
- A pull request may skip heavy Molehill qualification only when the repository CI change classifier proves every current/previous changed path is safe lowercase-Markdown under docs/**. Mixed, empty, malformed, non-Markdown, root-doc, workflow, test, runtime, package, data or unknown changes fail closed to requiring exact-head tlas-local-e2e.
...
- Synology must not run the 60-scenario full PR matrix, broad stress matrices, soak, performance depth or visual-regression depth as a substitute for Molehill-PC capacity.
