...
test('repository retirement authority preserves the audited bounded wave census',()=>{
  ...
  assert.equal(manifest.waves['lr2-verification-contracts'].supportModify.length,4);
  ...
  assert.equal(manifest.waves['lr4-governance-cleanup'].supportModify.length,14);
});
