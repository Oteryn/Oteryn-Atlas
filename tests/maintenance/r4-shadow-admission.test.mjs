...
function fixture(t,{layer=false}={}){
 ...
 if(layer){for(const name of ['verification-catalog.json','impact-manifest.json','protected-scenario-inventory.json'])put(trusted,'tools/verification/'+name,fs.readFileSync(path.join(root,'tools/verification',name)));put(trusted,active,fs.readFileSync(path.join(root,active)));put(trusted,'e2e/tests/layer-audit-desktop.spec.mjs',fs.readFileSync(path.join(root,'e2e/tests/layer-audit-desktop.spec.mjs')));}
 ...
 const base=git(trusted,'rev-parse','HEAD');git(trusted,'worktree','add','-q','-b','candidate',candidate,'HEAD');if(layer)fs.appendFileSync(path.join(candidate,'e2e/tests/layer-audit-desktop.spec.mjs'),'\n// Exact protected S2 owner selection.\n');else put(candidate,active,fs.readFileSync(path.join(trusted,template)));
...
test('admitted S2 spec modification resolves exactly one protected fixture browser owner for PR and MQ',t=>{
 const f=fixture(t,{layer:true}),results=f.invoke(),spec='e2e/tests/layer-audit-desktop.spec.mjs';
 for(const [index,result] of results.entries()){
  assert.equal(result.status,0,result.stderr);
  const gate=JSON.parse(result.stdout);
  assert.equal(gate.mode,'verification-restoration-r1-r3');
  assert.deepEqual(gate.changedPaths,[spec]);
  assert.equal(git(f.candidate,'diff','--name-status',gate.baseSha,gate.headSha),`M\t${spec}`);
  const candidate={repository:'Oteryn/Oteryn-Atlas',prNumber:index===0?7:null,baseSha:gate.baseSha,headSha:gate.headSha,treeSha:git(f.candidate,'rev-parse',gate.headSha+'^{tree}'),changedFiles:[{path:spec,status:'modified'}]};
  const {plan}=planShadow({candidate,root:f.candidate,protectedRoot:f.trusted});
  assert.deepEqual(plan.groups.map(group=>group.id),['e2e.layer-availability']);
  assert.deepEqual(plan.requiredDataCapabilities,['qualification_fixture']);
  assert.equal(plan.groups[0].executionEngine,'playwright');
  assert.deepEqual(plan.groups[0].specs,[spec]);
  assert.deepEqual(plan.groups[0].projects,['desktop-chromium']);
  assert.deepEqual(plan.stableTestIds,[`desktop-chromium::${spec}::layer controls expose correct availability and fail-closed states`]);
  assert.deepEqual(plan.requiredVisualGroupIds,[]);
 }
