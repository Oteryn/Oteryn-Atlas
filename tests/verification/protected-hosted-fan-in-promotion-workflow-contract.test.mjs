...
const workflowPath = path.join(ROOT, '.github/workflows/protected-hosted-fan-in-promotion.yml');
const legacyWorkflowPath = path.join(ROOT, '.github/workflows/legacy-molehill-transition-qualification.yml');

test('protected hosted fan-in promotion is bounded GitHub-hosted proof and never Molehill E2E', () => {
  assert.equal(fs.existsSync(workflowPath), true, 'protected hosted fan-in promotion workflow must exist');
  ...
  assert.match(workflow, /fix\/issue-179-protected-hosted-fan-in-promotion/);
  assert.doesNotMatch(heavy, /fix\/issue-179-protected-hosted-fan-in-promotion/);
  assert.match(workflow, /runs-on:\s*ubuntu-24\.04/);
  assert.doesNotMatch(workflow, /group:\s*atlas-runners|labels:\s*oteryn-atlas-pc/);
  assert.match(workflow, /Require exact four-file fan-in promotion delta/);
  assert.match(workflow, /protected-hosted-fan-in\.test\.mjs/);
