...
test('nightly browser depth has a bounded budget for its depth-only workload', () => {
  ...
  assert.ok(Number(match[1]) >= 180, `nightly browser-depth timeout ${match[1]}m is below the measured runner budget for the depth-only workload`);
});
