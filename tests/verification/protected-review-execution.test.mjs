...
test('protected producer executes complete hosted capture plan without claiming review approval',()=>{const p=plan();assert.equal(p.requiredFrames.length,30);assert.deepEqual(p.requiredFrames,read('protected-visual-capture-contract').requiredFrames);assert.ok(p.review.length);assert.doesNotThrow(()=>validateExecutionPlan(p));});
