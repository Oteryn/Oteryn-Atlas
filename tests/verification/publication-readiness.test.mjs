...
test('publication readiness validates bytes then atomically publishes an exact complete manifest', () => {
...
test('publication consumer rejects stale identity and any byte mutation before browser execution', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'atlas-publication-'));
  const source = path.join(root, 'source');
  const destination = path.join(root, 'published');
  fs.mkdirSync(source);
  ...
  assert.throws(() => validateReadyPublication({ publicationDir: destination, manifest, ...identity(), candidateSha: 'd'.repeat(40) }), /candidate/i);
  fs.rmSync(root, { recursive: true, force: true });
});
