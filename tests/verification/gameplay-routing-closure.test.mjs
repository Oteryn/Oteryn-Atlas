...
for (const path of [runtime, source]) test(`${path} preserves functional and bounded proof without FullWorld`, () => {
  const result = plan([{ path }]);
  assert(result.requiredGroupIds.includes('integration.source-contract'));
  assert(result.requiredGroupIds.includes('e2e.creatures'));
  ...
  assert(result.groups.find(g => g.id === 'integration.source-contract').specs.includes(source));
});
...
test('gameplay binding preserves overlapping protected smoke and visual obligations', () => {
  ...
  assert(result.requiredVisualGroupIds.includes('visual.creatures'));
});
