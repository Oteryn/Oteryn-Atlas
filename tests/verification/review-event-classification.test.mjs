...
test('review payload without changed_files uses complete live metadata',async()=>{
...
}))test(`live review readback rejects ${name}`,async()=>{const f=fixture();mutate(f.state);await assert.rejects(f.classify());});
