...
test('hosted qualification publication is built, atomically readied and served from exact local bytes', async () => {
  await withReadyServer(async ({ prepared, server }) => {
    ...
    assert.deepEqual(Object.keys(prepared.qualificationTrust).sort(), [
      'dataCapability', 'fixtureId', 'marker', 'minimapRoot', 'overviewRoot', 'pixelBucketRoot',
      'pixelRoot', 'productDigest', 'publicationRoot', 'runtimeIndexRoot', 'semanticRoot', 'sourceFingerprint',
    ].sort());
    assert.equal(prepared.qualificationTrust.dataCapability, 'qualification_fixture');
    assert.equal(prepared.qualificationTrust.minimapRoot, prepared.fixtureManifest.minimapRoot);
    assert.equal(prepared.qualificationTrust.productDigest, prepared.fixtureManifest.productDigest);
