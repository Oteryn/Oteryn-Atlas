...
test('read-only depth planning retains exact candidate identity',()=>{
 const planned=snapshot();
 assert.equal(assertCandidateReadback({planned,current:structuredClone(planned),...source()}),true);
 for(const key of ['headSha','baseSha','treeSha'])assert.throws(()=>assertCandidateReadback({planned,current:{...planned,[key]:'f'.repeat(40)},...source()}),/readback/);
});
