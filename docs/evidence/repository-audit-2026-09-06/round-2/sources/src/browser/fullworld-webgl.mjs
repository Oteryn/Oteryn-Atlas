...
export function createFullWorldWebGLRenderer(canvas,pixelCatalog,runtimePixelCatalog,options={}){
  ...
  const gpuPlacements=new Map(),uploadedBuckets=new Map();let nextTexel=0,records=[],instanceCount=0,instanceBufferBytes=0,residentPixelBytes=0,totalUploadMs=0,latestGpuRenderMs=null,renderGeneration=0;
  ...
  function render(view){
    const started=performance.now(),dpr=resizeCanvas(canvas);
    pollGpuTimers();
    gl.viewport(0,0,canvas.width,canvas.height);gl.clear(gl.COLOR_BUFFER_BIT);gl.useProgram(program);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D_ARRAY,texture);
