...
export function createFullWorldWebGLRenderer(canvas,pixelCatalog,runtimePixelCatalog,options={}){
  ...
  const totalLayers=Math.ceil(runtimePixelCatalog.manifest.counts.bytes/PAGE_BYTES); requireValue(totalLayers>=1&&totalLayers<=maxArrayLayers,`runtime pixel set requires ${totalLayers} layers; GPU exposes ${maxArrayLayers}`);
