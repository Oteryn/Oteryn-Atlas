...
Scheduled depth is defined by `.github/workflows/verification-depth.yml` and is additive to the 48-scenario exact-head PR gate rather than a second copy of it on the self-hosted nightly runner. `ATLAS_E2E_DEPTH=nightly` adds the extra DPR/tablet projects while the PR-gated required suite remains unchanged. The workflow runs the fixed stress seed matrix, repeated critical geometry/render scenarios, and stable worker-delivered performance/visual/accessibility/race/soak specs. Missing optional depth categories are recorded with explicit reasons in `optional-depth-skips.json`.
