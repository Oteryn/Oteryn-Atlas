...
test('mobile FullWorld exposes drawers and semantic navigation', async ({ page }) => {
  ...
  await mobileSearch.fill((__atlasQualification ? "Fixture Guide" : 'Sam'));
  const results = page.locator('#semantic-search-results-mobile');
  await expect(results).toBeVisible();
  const sam = results.locator('.semantic-search-result').filter({ hasText: (__atlasQualification ? "Fixture Guide" : 'Sam') }).first();
  ...
  await expect(page.locator('#inspector-content')).toContainText((__atlasQualification ? "Fixture Guide" : 'Sam'));
