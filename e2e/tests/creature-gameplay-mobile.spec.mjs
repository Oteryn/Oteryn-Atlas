...
test('mobile qualification NPC tap reaches readable Gameplay data and touch-sized tabs', async ({ page }, testInfo) => {
  const runtime = captureRuntimeFailures(page);
  await openFixture(page, GUIDE);
  ...
  await expect(page.locator('#creature-inspector')).toContainText(`Entity: ${GUIDE.entityId}`);
  await gameplayTab.tap();
  await expect(page.locator('#gameplay-section-sells')).toContainText('Fixture Rope');
  await expect(liveTab).toBeDisabled();
  const gameplayMetrics = await assertUserVisibleSurface(page, {
    label: 'Mobile qualification creature Gameplay inspector',
    elements: [
  ...
  await captureUserVisualEvidence(page, testInfo, 'mobile.creature-gameplay', {
    ...
    note: 'Mobile Gameplay drawer using Atlas-owned synthetic qualification data; no Game-owned fact is asserted here.',
  });
  assertNoRuntimeFailures(runtime);
});

test('mobile qualification monster renders loot/stats and keeps topmost Escape behavior', async ({ page }) => {
  const runtime = captureRuntimeFailures(page);
  await openFixture(page, SENTINEL);
