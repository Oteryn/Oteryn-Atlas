...
async function expectOpenDrawer(panel, toggle, { migrated, migratedName, legacyName }) {
  await expect(toggle).toHaveAttribute('aria-expanded', 'true');
  if (migrated) {
    await expect(toggle).toHaveAccessibleName(migratedName);
    expect(await panel.getAttribute('aria-hidden'), 'migrated open drawer must remove aria-hidden').toBeNull();
  } else {
    await expect(toggle).toHaveAccessibleName(legacyName);
    await expect(panel).toHaveAttribute('aria-hidden', 'false');
  }
  expect(await panel.evaluate((element) => element.inert), 'open mobile drawer must remain interactive').toBeFalsy();
}
...
async function expectMobileSearchMigrationState(page) {
  ...
  await expect(input).toHaveAccessibleName('Global semantic Atlas search');
  const role = await input.getAttribute('role');
  if (role === 'combobox') {
    await expect(input).toHaveAttribute('aria-autocomplete', 'list');
    await expect(input).toHaveAttribute('aria-controls', 'semantic-search-results-mobile-list');
    await expect(input).toHaveAttribute('aria-expanded', 'false');
    const popup = page.locator('#semantic-search-results-mobile-list');
...
test('mobile drawers expose truthful hidden state and restore keyboard focus', async ({ page }) => {
  ...
  await expectOpenDrawer(controlsPanel, controlsToggle, {
    migrated,
    migratedName: 'Hide Atlas controls',
    legacyName: 'Open Atlas controls',
  });
  ...
  await page.keyboard.press('Enter');
  await expectOpenDrawer(inspectorPanel, inspectorToggle, {
    migrated,
    migratedName: 'Hide inspector',
    legacyName: 'Open inspector',
  });
