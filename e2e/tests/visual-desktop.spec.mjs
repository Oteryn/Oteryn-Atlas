...
test('playback changes only verified animated presentation regions and restores static pixels', async ({ page }, testInfo) => {
  ...
  await page.addInitScript(() => {
    ...
    CanvasRenderingContext2D.prototype.drawImage = function (...args) {
      if (this.canvas?.id === 'animation-overlay') {
        const source = args[0]; const x = Number(args[1]); const y = Number(args[2]);
        const width = Number(args.length >= 5 ? args[3] : source?.width); const height = Number(args.length >= 5 ? args[4] : source?.height);
  ...
  await testInfo.attach('playback-pixel-metrics', { body: Buffer.from(JSON.stringify({ ...comparison, maskPng: undefined, diffPng: undefined, rectangles, alphaSamples }, null, 2)), contentType: 'application/json' });
