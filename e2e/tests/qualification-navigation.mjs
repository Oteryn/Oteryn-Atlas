...
function parseTrust(raw) {
  ...
  let descriptor;
  try { descriptor = JSON.parse(raw); } catch (error) { throw new TypeError(`ATLAS_QUALIFICATION_TRUST_JSON is invalid JSON: ${error.message}`); }
  if (!descriptor || typeof descriptor !== 'object' || Array.isArray(descriptor)) throw new TypeError('ATLAS_QUALIFICATION_TRUST_JSON must encode an object');
  const fields = Object.keys(descriptor).sort();
  if (JSON.stringify(fields) !== JSON.stringify(EXACT_FIELDS)) throw new TypeError('qualification trust descriptor fields mismatch');
  for (const field of ROOT_FIELDS) if (!SHA256.test(descriptor[field] ?? '')) throw new TypeError(`qualification trust ${field} is invalid`);
  if (descriptor.marker === BOUNDED_MARKER && descriptor.fixtureId === BOUNDED_FIXTURE_ID && descriptor.dataCapability === 'bounded_real_world') return Object.freeze({ mode: 'bounded', descriptor: Object.freeze({ ...descriptor }) });
  if (descriptor.marker !== QUALIFICATION_MARKER || descriptor.fixtureId !== QUALIFICATION_FIXTURE_ID || descriptor.dataCapability !== 'qualification_fixture') throw new TypeError('qualification trust identity mismatch');
...
function validateSemanticIndex(index, trust) {
  ...
  if (index.source.contract_id !== 'oteryn-atlas-qualification-fixture-v1' || index.source.capability !== 'qualification-semantic-search-v1') throw new TypeError('qualification semantic source contract mismatch');
  ...
  if (!navigation || typeof navigation !== 'object' || Array.isArray(navigation)
    ...
    || navigation.record_id !== QUALIFICATION_DEFAULT_NAVIGATION.record_id) throw new TypeError('qualification semantic default navigation identity mismatch');
  ...
  if (defaults.length !== 1) throw new TypeError(`qualification semantic index must contain exactly one default navigation record, observed ${defaults.length}`);
  const record = defaults[0];
  const position = record?.position;
  if (!Array.isArray(record?.capabilities) || !record.capabilities.includes('navigation')
    || !Number.isSafeInteger(position?.x) || !Number.isSafeInteger(position?.y) || !Number.isSafeInteger(position?.floor)) throw new TypeError('qualification semantic default navigation record is not navigable');
  return record;
}

const SHARED_HISTORICAL_DEFAULT = Object.freeze({ x: '32369', y: '32241', floor: '-7' });
const CANONICAL_FULLWORLD_PATH = '/web/fullworld.html';
const CANONICAL_ATLAS_ORIGINS = Object.freeze(['http://atlas-web:8080', 'https://atlas-web:8080']);
...
function serializeEntry(url, isRelative) { return isRelative ? `${url.pathname}${url.search}${url.hash}` : url.href; }
function isCanonicalSerializedHistoricalDefault(entry, url) {
  const canonical = new URL(url.href);
  for (const [field, value] of Object.entries(SHARED_HISTORICAL_DEFAULT)) canonical.searchParams.set(field, value);
...
function isSharedHistoricalDefault(entry) {
  ...
  let url;
  try { url = new URL(entry, 'http://atlas.invalid'); } catch { return false; }
  if (url.pathname !== CANONICAL_FULLWORLD_PATH || !isCanonicalSerializedHistoricalDefault(entry, url)) return false;
...
function rewriteEntry(entry, position) {
  ...
  if (url.pathname !== CANONICAL_FULLWORLD_PATH) throw new TypeError(`qualification navigation target is not Atlas FullWorld: ${url.pathname}`);
